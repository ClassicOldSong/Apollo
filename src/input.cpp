...
////////////////////////////////////////// START ALTERNATE GAMEPAD NUMBERING ATTEMPT 1
#define ALTGAMEPADUSERPREFIX "DeviceName"
#define ALTGAMEPADORDERPREFIX "Order"
#define ALTGAMEPADCONNECTOR "="
#define ALTGAMEPADENCAPSULATE "\""
#define ALTGAMEPADBEGINCHARACTER " "
#define ALTGAMEPADEMPTY ""

#define ALTGAMEPADALLNONLISTED "NOMATCH"

//#define ALTGAMEPADANYWITHIN 999
//#define ALTGAMEPADANYWITHOUT 1000
...
#define ALTGAMEPADMAXSTRINGNUMBER 99
...
int parseAlternativeGamepadNumberingString( std::string sLine, int iMin, int iMax ) {
  ...
  sLineToSearch = ALTGAMEPADEMPTY;
  sLineToSearch += ALTGAMEPADBEGINCHARACTER;
  sLineToSearch += sLine;



  for (iLoop = 1; iLoop < ALTGAMEPADMAXSTRINGNUMBER; iLoop++) {
    sOrder.clear();
    sDeviceName.clear();

    // Build the string to look for
    // Find the DeviceName
    sSearch1 = ALTGAMEPADEMPTY;
    sSearch1 += ALTGAMEPADBEGINCHARACTER;
    sSearch1 += ALTGAMEPADUSERPREFIX;
    sSearch1 += std::to_string(iLoop);
    sSearch1 += ALTGAMEPADCONNECTOR;
    sSearch1 += ALTGAMEPADENCAPSULATE;

    sSearch2 = ALTGAMEPADEMPTY;
    sSearch2 += ALTGAMEPADBEGINCHARACTER;
    sSearch2 += ALTGAMEPADORDERPREFIX;
    sSearch2 += std::to_string(iLoop);
    sSearch2 += ALTGAMEPADCONNECTOR;
    sSearch2 += ALTGAMEPADENCAPSULATE;

    iPosition1 = sLineToSearch.find(sSearch1);
    if (iPosition1 != std::string::npos ) {
      sSearch3 = ALTGAMEPADENCAPSULATE;
        ...
        if (iPosition3 != std::string::npos) {
          ...
          if(iPosition4 != std::string::npos) {
            ...
            while (sOrderWorking.length() > 0) {
              ...
              iPosition5 = sOrderWorking.find(ALTGAMEPADBEGINCHARACTER);
              if (iPosition5 == 0) {
            ...
            if (vCurrent.vOrder.size() > 0) {
              ...
              vCurrent.sDeviceName = ALTGAMEPADEMPTY;
              vCurrent.sOrder = ALTGAMEPADEMPTY;
...
int matchAlternativeGamepadNumberingString(std::string sDeviceName, struct sDeviceNameOrder &sReturn ) {
  ...
  if (bFound == true) {
    ...
    return 0;
      ...
      // If there are no matches, try to the ALTGAMEPADALLNONLISTED name and use that to find the next match
      if( sDeviceName == ALTGAMEPADALLNONLISTED ) {
        return -1;
      } else {
        return matchAlternativeGamepadNumberingString( ALTGAMEPADALLNONLISTED, sReturn );
      }
  }
}
////////////////////////////////////////// END ALTERNATE GAMEPAD NUMBERING
namespace input {
...
// This is for the alternate gamepad numbering
  ...
  int altgamepadnumberingDeleteGamepad( std::shared_ptr<input_t> &input, int controllerNumber) {
  ...
  int altgamepadnumberingAssignGamepad( std::shared_ptr<input_t> &input, int controllerNumber, platf::gamepad_arrival_t &arrival ) {
    ...
    if( config::input.enable_alt_controller_numbering_mode == true ) {
      ...
      if( input->iAltControllerNameIndex != -1 ) {
        ...
        if( config::alt_gamepad_numbering.sDeviceNames.size() > input->iAltControllerNameIndex &&
  ...
  int altgamepadnumberingInitialPlaceholderAllocation(std::shared_ptr<input_t> &input, platf::gamepad_arrival_t &arrival, int iControllerNumber ) {
    ...
    if( config::input.enable_alt_controller_numbering_mode == true ) {
      ...
      if( config::alt_gamepad_numbering.bFirstTimeControllerAllocation == true ) {
        ...
        if( (config::input.alt_controller_order_string).length() <= 0 ) {
          parseAlternativeGamepadNumberingString("DeviceName1=\"NOMATCH\" Order1=\"999\"", 1, config::input.alt_controller_count);
        } else {
          parseAlternativeGamepadNumberingString(config::input.alt_controller_order_string, 1, config::input.alt_controller_count );
        }
        ...
        while( iPlaceholdersAllocated < iNumberOfPlaceholders ) {
          ...
          if( iIndexPlaceholder != -1 ) {
            ...
            if( iOld_OS != -1 ) {
              ...
              iReturn = platf::alloc_gamepad(platf_input, { iOld_OS, (uint8_t)(((uint8_t)MAX_GAMEPADS)-(1)-((uint8_t)iLoopCount)), ((uint8_t)iLoopCount)}, arrival, config::placeholder_feedback_queues[iOld_OS] );
  ...
  void passthrough(std::shared_ptr<input_t> &input, PSS_CONTROLLER_ARRIVAL_PACKET packet) {
    ...
    if( (config::input.enable_alt_controller_numbering_mode == true) && (config::alt_gamepad_numbering.bFirstTimeControllerAllocation == true) && (input->gamepads[packet->controllerNumber].alt_controller_placeholder != 4) ) {
      ...
      altgamepadnumberingInitialPlaceholderAllocation(input, arrival, packet->controllerNumber );
    }
    if( config::input.enable_alt_controller_numbering_mode == true && input->gamepads[packet->controllerNumber].alt_controller_placeholder != 4) {
      // Assign gamepad to the session
      iReturn = altgamepadnumberingAssignGamepad( input, packet->controllerNumber, arrival );
    }

    if( config::input.enable_alt_controller_numbering_mode == false || iReturn == ALT_CONTROLLER_NO_PLACEHOLDER ) {
  ...
  void passthrough(std::shared_ptr<input_t> &input, PNV_MULTI_CONTROLLER_PACKET packet) {
    ...
    if ((packet->activeGamepadMask & (1 << packet->controllerNumber)) && gamepad.id < 0) {
      ...
      if( (config::input.enable_alt_controller_numbering_mode == true) && ( gamepad.alt_controller_placeholder == 2 || gamepad.alt_controller_placeholder == 3 ) ) {
        altgamepadnumberingDeleteGamepad( input, packet->controllerNumber);
      } else {

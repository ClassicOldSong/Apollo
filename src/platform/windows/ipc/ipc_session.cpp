...
namespace platf::dxgi {
  ...
  void ipc_session_t::initialize_if_needed() {
    ...
    std::filesystem::path exe_path = mainExeDir / L"tools" / L"apollo_wgc_capture.exe";
    ...
    if (!_process_helper->start(exe_path.wstring(), arguments)) {
      ...
      BOOST_LOG(error) << "Failed to start apollo_wgc_capture executable at: " << exe_path.wstring()

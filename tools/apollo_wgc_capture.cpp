/**
 * @file apollo_wgc_capture.cpp
 * @brief Windows Graphics Capture helper process for Apollo.
 *
 * This standalone executable provides Windows Graphics Capture functionality
 * for the main Apollo streaming process. It runs as a separate process to
...
public:
   ...
   * Uses the same D3D11_CREATE_DEVICE_FLAGS as the main Apollo process.
   *
 ...
 * Apollo process. It creates shared textures with keyed mutexes for synchronization
...
std::string get_temp_log_path() {
  ...
  if (auto len = GetTempPathW(MAX_PATH, temp_path.data()); len == 0 || len > MAX_PATH) {
    ...
    return "apollo_wgc_helper.log";
  }
  temp_path.resize(wcslen(temp_path.data()));  // Remove null padding
  std::wstring wlog = temp_path + L"apollo_wgc_helper.log";
  // Convert to UTF-8
 ...
 *
 * Processes incoming messages from the main Apollo process via named pipe:
 *
 ...
 *
 * This standalone executable serves as a helper process for Apollo's Windows Graphics
 ...
 * 2. **IPC Setup**: Establishes named pipe communication with the main Apollo process
